...
REGISTER_OP("AcousticOneStepGrad")
...
#if 0
/*-------------------------------------------------------------------------------------*/
class AcousticOneStepOp : public OpKernel {
private:
  
public:
  explicit AcousticOneStepOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {    
    ...
    // create output shape
    int NX = *nx.flat<int64>().data();
    int NY = *ny.flat<int64>().data();
...
REGISTER_KERNEL_BUILDER(Name("AcousticOneStep").Device(DEVICE_CPU), AcousticOneStepOp);



class AcousticOneStepGradOp : public OpKernel {
private:
  
public:
  explicit AcousticOneStepGradOp(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("AcousticOneStepGrad").Device(DEVICE_CPU), AcousticOneStepGradOp);
#endif 
...
#ifdef GOOGLE_CUDA
class AcousticOneStepOpGPU : public OpKernel {
private:
  
public:
  explicit AcousticOneStepOpGPU(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {    
    ...
    // create output shape
    int64 NX, NY;
    cudaMemcpy(&NX, nx.flat<int64>().data(), sizeof(int64), cudaMemcpyDeviceToHost);
    cudaMemcpy(&NY, ny.flat<int64>().data(), sizeof(int64), cudaMemcpyDeviceToHost);
    ...
    // TODO:
    double HX, HY, DT;
    cudaMemcpy(&HX, hx_tensor, sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&HY, hy_tensor, sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&DT, dt_tensor, sizeof(double), cudaMemcpyDeviceToHost);

    AcousticOneStepForward(w_tensor,
                            wold_tensor,
                            phi_tensor,
                            psi_tensor,
                            sigma_tensor,
...
REGISTER_KERNEL_BUILDER(Name("AcousticOneStep").Device(DEVICE_GPU), AcousticOneStepOpGPU);

class AcousticOneStepGradOpGPU : public OpKernel {
private:
  
public:
  explicit AcousticOneStepGradOpGPU(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("AcousticOneStepGrad").Device(DEVICE_GPU), AcousticOneStepGradOpGPU);
