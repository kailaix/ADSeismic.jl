...
num_layer = 4
Δvp = Generator(z, isTrain, dropout_rate, w0=ceil(Int, size(vp0)[1]/(2^num_layer)), h0=ceil(Int, size(vp0)[2]/(2^num_layer)), num_layer=num_layer, vmin=-2.5, vmax=2.5)
...
function callback(_, iter, loss)
  ...
  if iter%10==0
    ...
    clf()
    pcolormesh([0:params.NX+1;]*params.DELTAX/1e3,[0:params.NY+1;]*params.DELTAY/1e3,  vp_std', shading="auto", cmap="jet")
    axis("scaled")
    colorbar(shrink=0.4)
    xlabel("x (km)")
    ylabel("z (km)")
...
for iter = 0:max_iter
  ...
  # end
  lr = 1e-3
  t = @elapsed  _, ls = run(sess, [opt, loss], feed_dict=Dict(lr_decayed=>lr, isTrain=>true))
